...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resp = await fetch(
      `${URL}/rest/v1/restaurants?select=id,name,is_active,service_fee,address,sort_order,cover_url,menu_items(id,name,description,price,category,is_available,sort_order,image_url,menu_item_images(id,url,sort_order))&order=sort_order.desc`,
      { headers }
    );
    if (!resp.ok) {
